<script setup>
import { reactive, computed, toRef, ref, watchEffect, onMounted } from "vue";
...
const handleSubmit = async () => {
  ...
  try {
    const response = await apiService.simpanIdentitas(form);
    if (response.data.success) {
      await alertSuccess(
        "Berhasil",
        "Data identitas pasien berhasil disimpan ke Database.",
        2000,
      );
    ...
    if (error.response) {
      ...
      alertError("Gagal", pesan);
    } else {
      alertError("Koneksi Error", "Tidak dapat terhubung ke server.");
    }
  } finally {
...
<template>
  <div class="min-h-screen bg-gray-50 flex flex-col font-sans print:hidden">
    <div class="sticky top-0 z-50 print:hidden">
      <UserHeader :user-name="namaUser" />
    </div>
    <div class="flex-1 py-10 px-4 sm:px-4 lg:px-8 print:hidden">
      <div
      ...
      >
        ...
        >
          <div class="text-center mb-6">
            <h1
              class="text-3xl font-bold text-gray-900 tracking-tight border-b border-gray-200 pb-4"
            >
        ...
        >
          ...
          >
            <div class="lg:col-span-4">
              <BaseInput v-model="form.noKtp" label="NIK / No. KTP" />
            </div>
              ...
              <div class="flex">
                ...
                <div class="ml-3">
                  <p class="text-sm text-yellow-700">
                    ...
                    benar dan dapat dipertanggungjawabkan. Saya bersedia
                    mengikuti peraturan yang berlaku di RS Kristen Ngesti
                    Waluyo.
                  </p>
                </div>
              </div>
